...
public class BattleField extends Applet implements Runnable, MouseListener,
		MouseMotionListener {
	/**
	 * 
	 */
	...
	public static final int nbTeam = 1;
	...
	public void initSurface() {
		...
		for (int i = 0; i < nbPolyline; i++) {
			PolylineObject ob = new PolylineObject(new Vector2d(r.nextFloat()
					* surface.wxsize, r.nextFloat() * surface.wysize), surface);
			...
			Vector2d northEast = new Vector2d(northOuest.x + width,
					northOuest.y);
			Vector2d SouthOuest = new Vector2d(northOuest.x, northOuest.y
					+ height);
			Vector2d southEast = new Vector2d(northOuest.x + width,
					northOuest.y + height);
		...
		// PolylineObject ob1 = new PolylineObject(new
		// Vector2d(100F,200F),surface);
		...
		//
		// PolylineObject ob2 = new PolylineObject(new
		// Vector2d(300F,500F),surface);
		...
		//
		// PolylineObject ob3 = new PolylineObject(new
		// Vector2d(310F,400F),surface);
	...
	public void paint(Graphics g) {
		...
		/*
		 * if ( (pointA.x > -1) && (pointB.x > -1) ) { gui_string =
		 * "Il va falloir modifier tout cela pour en faire un jeu... ["; if
		 * (surface.cansee(pointA, pointB)) {
		 * buffer_canvas.setColor(Color.green); gui_string += "A voit B"; } else
		 * { buffer_canvas.setColor(Color.red); gui_string += "A ne voit pas B";
		 * } gui_string +="]"; buffer_canvas.drawLine((int)pointA.x,
		 * (int)pointA.y, (int)pointB.x, (int)pointB.y); }
		 */
		drawHUD();
		showbuffer();
	}
	...
	private void drawHUD() {
		...
		buffer_canvas.drawChars(gui_string.toCharArray(), 0,
				Math.min(80, gui_string.length()), 22, viewer_ysize - 7);
	}
